...
const overlayHtml = fs.readFileSync(path.resolve(__dirname, 'overlay.html'), 'utf-8');
...
fastify.get('/proxy', async (request: FastifyRequest<{ Querystring: { url: string } }>, reply) => {
  try {
    ...
    // Modify the response body to include the custom overlay
...
const start = async () => {
  try {
    await fastify.listen({ port: 1414, host: '0.0.0.0' });
    console.log('Proxy server is running at http://localhost:1414');
  } catch (err) {
    console.error(err);
    process.exit(1);
  }
